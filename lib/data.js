...
const getData = async function (game_id) {
    ...
    for (const border of raw_details._content.board._content.border) {
        ...
        if (territory_a && territory_b) {
            territory_a.can_attack.push(territory_b);
            territory_b.will_defend.push(territory_a);
        }
    }
    data.map.continents = [];
    for (const raw_continent of raw_details._content.continents._content.continent) {
        ...
        for (const territory_id of raw_continent.cids.split(',').map(c => parseInt(c, 10))) {
            ...
            if (territory) {
                continent.territories.push(territory);
            }
        }
    ...
    for (const area of raw_state._content.board._content.area) {
        ...
        const player = findSeat(data, parseInt(area.playerid, 10));
        if (territory && player) {
            territory.player = player;
            territory.units = parseInt(area.units, 10);
            player.territories.push(territory);
        } else {
            console.log('failed to find something', area.id, !!territory, area.playerid, !!player);
        }
    }
...
const getHistory = async function (game_id) {
    ...
    const [data, raw_history] = await Promise.all([
        getData(game_id),
        _getHistory(game_id, 0),
    ]);
    const history = [];
    for (const move of raw_history) {
        ...
        maybeSetInt(entry, 'player', move, 's', (seat_id) => findSeat(data, seat_id));
        maybeSetInt(entry, 'territory', move, 'cid', (territory_id) => findTerritory(data, territory_id));
        ...
        maybeSetInt(entry, 'to_territory', move, 'tcid', (territory_id) => findTerritory(data, territory_id));
        maybeSetInt(entry, 'from_territory', move, 'fcid', (territory_id) => findTerritory(data, territory_id));
        maybeSetInt(entry, 'eliminated_player', move, 'es', (seat_id) => findSeat(data, seat_id));
        maybeSetInt(entry, 'team_id', move, 'tid');
        maybeSetIntList(entry, 'players', move, 'slist', (seat_id) => findSeat(data, seat_id));
